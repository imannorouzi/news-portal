...
export class EditableMeetingComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    // this.post.times.push(new DateTime());
  }
  ...
  onSubmit(){
    if(this.validateForm()) {
      if (this.event.venue && this.event.venue.id === 0) {
        // this.post.userId = this.authService.getUser().id;
      ...
      this.dataService.postMeeting(this.event).subscribe(
        (value:any) => {
          ...
          this.navigationService.navigate("/posts");
  ...
  onImportClick(event) {
    ...
    // this.contactsModal.setSelected(this.post.attendees);
    event.preventDefault();
  }
  addContact(event) {
